...
function resolveVariantPrice(productPrice, variant) {
  if (typeof variant.priceOverride === 'number') return variant.priceOverride;
  if (typeof variant.priceDelta === 'number') return Number(productPrice) + Number(variant.priceDelta);
...
export async function addItem(userId, { productId, variantId, quantity = 1 }) {
  ...
  if (variantId) {
    ...
    if (shouldCheckStock) {
      available = await getAvailableStock(product._id, variantId);
    ...
    if (shouldCheckStock) {
      available = await getAvailableStock(product._id, null);
      if (available < quantity) throw errors.badRequest(ERROR_CODES.INSUFFICIENT_STOCK);
    }
  }
  ...
  const currentQuantity = idx >= 0 ? Number(cart.items[idx].quantity || 0) : 0;
  if (available < currentQuantity + quantity) {
